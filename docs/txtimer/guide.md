...
* [Source Code](https://github.com/shepherdviolet/glacimon/tree/master/glacispring-txtimer/src/main/java/com/github/shepherdviolet/glacimon/spring/x/monitor/txtimer)
...
```text
-Dglacispring.txtimer.enabled=false
```
...
* 日志关键字: `TxTimer`
* [日志样例](https://github.com/shepherdviolet/glacimon/blob/master/docs/txtimer/log-sample.md)
...
```text
    public static void main(String[] args) {
        ...
        System.setProperty("glacispring.txtimer.report.interval", 
                System.getProperty("glacispring.txtimer.report.interval", "10"));
        // 启动程序 ......
    }
```

* 默认情况下, 统计报告每隔五分钟输出一次, 时间间隔通过`glacispring.txtimer.report.interval`修改
...
> -Dglacispring.txtimer.report.printpermin=true false时, 报告的周期为N分钟(默认5分钟, N为日志打印间隔), true时, 报告的周期为1分钟(日志量变大, 数据变精细), 默认false<br>
...
* 默认情况下, 统计报告会输出所有交易的信息, 可以通过`glacispring.txtimer.threshold`系列参数配置输出的条件
* 以上面的配置为例, 当一个交易的平均耗时超过2000ms, 或最大耗时超过10000ms, 或最小耗时超过1000ms, 才会输出信息, 否则不输出
* 默认情况下, 全量统计报告关闭, 可以通过`glacispring.txtimer.reportall.interval`参数指定一个输出间隔来开启
* 以上面的配置为例, 每隔5分钟, 日志中会打印满足`glacispring.txtimer.threshold`系列参数条件的交易信息, 每隔60分钟, 日志中会打印所有交易的信息
...
> glacispring.txtimer.threshold.avg=2000 打印周期内平均耗时超过该值的交易, 单位:毫秒<br>
> glacispring.txtimer.threshold.max=10000 打印周期内最大耗时超过该值的交易, 单位:毫秒<br>
> glacispring.txtimer.threshold.min=1000 打印周期内最小耗时超过该值的交易, 单位:毫秒<br>
> glacispring.txtimer.reportall.interval=60 全量日志报告输出间隔(周期), 单位:分钟, 2-∞, 默认∞(不输出全量日志)<br>
> glacispring.txtimer.report.printpermin=true false时, 报告的周期为N分钟(默认5分钟, N为日志打印间隔), true时, 报告的周期为1分钟(日志量变大, 数据变精细), 默认false<br>
